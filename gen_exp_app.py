...
def build_angular_service():
    ...
    export class HttpService {
        ...
        get%(CONTROLLER_NAME_TITLE)s() {
            return this._http.get("/%(CONTROLLER_NAME)s");
        }

        get%(MODEL_NAME_TITLE)s(%(MODEL_NAME)sId) {
            return this._http.get(`/%(CONTROLLER_NAME)s/${%(MODEL_NAME)sId}`)
        }

        post%(MODEL_NAME_TITLE)s(%(MODEL_NAME)sObj) {
            return this._http.post("/%(CONTROLLER_NAME)s", %(MODEL_NAME)sObj)
        }

        put%(MODEL_NAME_TITLE)s(%(MODEL_NAME)sId, %(MODEL_NAME)sObj) {
            return this._http.put(`/%(CONTROLLER_NAME)s/${%(MODEL_NAME)sId}`, %(MODEL_NAME)sObj)
        }
        
        delete%(MODEL_NAME_TITLE)s(%(MODEL_NAME)sId) {
            return this._http.delete(`/%(CONTROLLER_NAME)s/${%(MODEL_NAME)sId}`)
        }
