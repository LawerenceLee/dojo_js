    ...
    quizValidation = quiz => {
        ...
        for (let question of quiz['questions']) {
            ...
            if (question['question'] === "") {
                errors.push("Question may not be blank.")
            } 
    ...
    module.exports = {
        index: (req, res) => {
            Quizzes.find({}, returnObjBuilder(res))
        },
        show: (req, res) => {
            Quizzes.findById(req.params.quizId, returnObjBuilder(res))
        },
        create: (req, res) => {
        ...
        update: (req, res) => {
            ...
            if (errors.length > 0) { res.json({ message: "error", error: errors }) }
                ...
                req.body.questionCount = req.body.questions.length;
                Quizzes.findByIdAndUpdate(req.params.quizId, req.body, {new: true}, returnObjBuilder(res))
            };
        },
        destroy: (req, res) => {
            Quizzes.findByIdAndRemove(req.params.quizId, returnObjBuilder(res))
        },
    }
